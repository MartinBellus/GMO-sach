...
class ChessboardUI(tkinter.Canvas):
        ...
        for r in range(BOARD_Y):
            for c in range(BOARD_X):
                self.squares[invert(r)][c] = super().create_rectangle(self.size_x*c + PADDING,    self.size_y*r + PADDING,
                                                                      self.size_x *
                                                                      (c+1) + PADDING, self.size_y *
                                                                      (r+1) +
                                                                      PADDING,
                                                                      fill=Color.DEFAULT[(invert(r)+c) % 2])

        for c in range(BOARD_X):
            super().create_text(self.size_x*(c+0.5) + PADDING,
                                self.height-PADDING/2, text=str(c+1))

        for r in range(BOARD_Y):
            super().create_text(PADDING/2, self.size_y*(r+0.5) +
                                PADDING, text=chr(ord('A') + invert(r)))
    ...
    def switch_state(self, new_state: int, *args):
        ...
        match new_state:
            case State.PRE_GAME:
                ...
                super().create_text(self.width/2, PADDING/2, text="Please select piece presets.",
                                    anchor="center", justify="center", tag="text")
            case State.GAME_START:
                # zacne hru
                super().create_text(self.width/2, PADDING/2, text="Game will start soon.",
                                    anchor="center", justify="center", tag="text")
            ...
            case State.WHITE_TURN:
                super().create_text(self.width/2, PADDING/2, text="White turn.",
                                    anchor="center", justify="center", tag="text")
            case State.BLACK_TURN:
                super().create_text(self.width/2, PADDING/2, text="Black turn.",
                                    anchor="center", justify="center", tag="text")
            case State.PROMOTE:
                super().create_text(self.width/2, PADDING/2, text="Black turn.",
                                    anchor="center", justify="center", tag="text")
            case State.GAME_OVER:
                print("game over")
                super().create_text(self.width/2, PADDING/2,
                                    text=args[0], anchor="center", justify="center", tag="text")
                self.bind("<Button-1>",
                          lambda x: self.switch_state(State.PRE_GAME))
            case State.LAB:
                super().create_text(self.width/2, PADDING/2, text="Welcome to LAB",
                                    anchor="center", justify="center", tag="text")
                self.bind("<Button-1>", self.ingame_click)
                self.redraw_pieces()
            case _:
    ...
    def ingame_click(self, event: tkinter.Event):
        click: Vector = Vector(int((event.x - PADDING)//self.size_x),
                               invert(int((event.y - PADDING)//self.size_y)))
        print(click.x, click.y)
        if not inside_chessboard(click):
            print("mimo")
            return
        current_descriptors: list[MoveDescriptor] = self.controller.get_moves(
            self.selected)
    ...
    def draw_moves(self, current_descriptors: list[MoveDescriptor]):
        for move in current_descriptors:
            ...
            super().itemconfig(
                self.squares[where.y][where.x], fill=Color.MOVE[where.parity()])
    ...
    def do_turn(self, move: MoveDescriptor):
        ...
        match gameState:
            ...
            case GameStatus.IN_PROGRESS:
                self.switch_state(
                    State.BLACK_TURN if self.controller.get_current_player == colors.BLACK else State.WHITE_TURN)
            case _:
                raise Exception("Wrong game status")

    def redraw_pieces(self):
        super().delete("piece")  # reset canvas
        state: list[list[None | PieceInfo]
                    ] = self.controller.get_board_for_reading()
        for r in range(BOARD_Y):
            for c in range(BOARD_X):
                if state[invert(r)][c] != None:
                    ...
                    img = ImageTk.PhotoImage(Image.open(image).resize(
                        (int(self.size_x), int(self.size_y))))
                    self.images.append(img)
                    super().create_image(PADDING + (c + 0.5)*self.size_x,
                                         PADDING + (r + 0.5)*self.size_y, image=img, tag="piece")

    def clear_selected(self, current_descriptors: list[MoveDescriptor]):
        for move in current_descriptors:
            where: Vector = move.to_position
            super().itemconfig(
                self.squares[where.y][where.x], fill=Color.DEFAULT[where.parity()])

    def place_piece(self, dna: str, x: int, y: int):
        self.controller._insert_piece(
            Piece(Genome(dna), colors.BLACK), Vector(x, y))
        self.redraw_pieces()
    ...
    def place_preset(self, preset: str):
        ...
        try:
            if len(parsed_preset) == 1:
                self.controller.load_preset(
                    Preset.fetch_preset(parsed_preset[0]), colors.WHITE)
            else:
                self.controller.load_preset(
                    Preset(parsed_preset), colors.WHITE)
            self.redraw_pieces()
        except Exception as ex:
            raise ex
